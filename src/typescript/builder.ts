...
const fieldInterfaceBuilder = (type: string, isArray: boolean, enumerator?: string[] | number[], items?: string, requiredFields?: string[]) => {
  if (enumerator) {
    if (type === "string") {
...
export const interfaceBuilder = (name: string, fields: Field[], requiredFields?: string[]) => `export interface ${name} {
  ...
  return `${allOrNothing(readOnly, 'readonly ')}${toCamelCase(field)}${allOrNothing(!(requiredFields && requiredFields.includes(field)), '?')}: ${fieldInterfaceBuilder(type, isArray, enumerator, items, requiredFields)}`
}).join('\n  ')}
}
`
...
export function endpointInterfaceBuilder(endpoints: Endpoint[], type?: string) {
  return (
    `
export interface I${toCamelCase(type)}Endpoints {${endpoints.map(endpoint => endpoint.actions.map(action => {
...
${
      tagNames.map(name =>
        `import { I${name}Endpoints, ${name}Endpoints } from './${name}'`
      ).join('\n')
      }
