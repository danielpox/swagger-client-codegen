...
export interface I${type}Endpoints {${endpoints.map(endpoint => endpoint.actions.map(action => {
        const responseDataType = action.responseDataType && utils_1.allOrNothing(action.responseDataType.dataType, `Promise<${action.responseDataType.array ? `${action.responseDataType.dataType}[]` : action.responseDataType.dataType}>`);
        return (`
...
export const ${type}Endpoints: I${type}Endpoints = {
  ${endpoints.map(endpoint => endpoint.actions.map(action => {
        const responseDataType = action.responseDataType && utils_1.allOrNothing(action.responseDataType.dataType, `<${action.responseDataType.array ? `${action.responseDataType.dataType}[]` : action.responseDataType.dataType}>`);
        return (`
  ${action.name}: async function (${retrieveEndpointParams({ pathParams: action.pathParams, queryParams: action.queryParams, bodyParams: action.bodyParams, includeType: false })}) {
    try {
      const response = await API.http.${action.type}${responseDataType}(\`${endpoint.path.replace(/({\w+})/g, (_, param) => `$${param}`)}\`${utils_1.allOrNothing(action.queryParams.length, ', queryParams')}${utils_1.allOrNothing(action.bodyParams.length, ', bodyParams')})
