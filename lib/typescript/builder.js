...
const isFile = (type) => type && type === 'file' ? 'File' : type;
...
exports.interfaceBuilder = (name, fields, requiredFields) => `export interface ${toCamelCase(name)} {
  ${fields
...
function retrieveEndpointParams({ pathParams, queryParams, bodyParams, includeType = true }) {
    return [
        pathParams &&
            utils_1.allOrNothing(pathParams.length, pathParams
                .map(param => `${param.name}${utils_1.allOrNothing(includeType, `${utils_1.allOrNothing(!param.required, "?")}: ${isFile(isNumber(param.type.type))}`)}`)
                .join(", ")),
        queryParams &&
    ...
    ${utils_1.allOrNothing(queryParams.length, queryParams
                .map(param => `${param.name}${utils_1.allOrNothing(!param.required, "?")}: ${isFile(isNumber(param.type.type))}${utils_1.allOrNothing(param.type.array, "[]")}`)
                .join(",\n    "))}
  }`)}`),
        bodyParams &&
